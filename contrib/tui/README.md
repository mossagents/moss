...
- **水平分隔线**：输入框上方渲染一条 `────` 线，通过 `renderComposerInput()` 渲染
...
- 斜杠弹窗最多显示 10 条候选；顶部紧凑 hint 最多显示 8 条
...
### InputConfirm（确认 / 审批）— 内联确认区

与 `InputSelect` 一样直接替换编辑器区域，不再打开覆盖层：

- 简单确认（`isSimpleConfirmAskActive`）：Yes/No 内联选项
- 审批请求（带 `Approval`）：工具信息 + 授权范围选项（Allow once / Session / Project / Deny）
- 按 `Enter` 立即提交当前决定
- 按 `Esc` 取消并中断当前 Agent 运行
...
| `overlayAsk` | 通用 Ask 表单（如 `InputForm` / 自由输入） |
| `overlayModel` | 模型选择器 |
| `overlayTheme` | 主题选择器 |
| `overlayHelp` | 命令帮助列表 |
...
> **InputSelect / InputConfirm 不使用覆盖层**，直接内联渲染到编辑器区域。
...
见 [`EXTENSIONS.md`](./EXTENSIONS.md) 了解 TUI 扩展 API，可添加：
- 状态栏 widget（`HeaderMetaWidgets`）
- 自定义键绑定（`KeyBindings`）
- Slash 命令（`SlashCommands`）
- 自定义 overlay（`Overlays`）
