...
func (w *ObjectStoreWorkspace) Stat(ctx context.Context, path string) (workspace.FileInfo, error) {
	meta, err := w.client.Head(ctx, w.key(path))
	if err != nil {
		return workspace.FileInfo{}, fmt.Errorf("workspace stat %q: %w", path, err)
	}
