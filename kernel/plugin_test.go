...
func TestPlugin_InstallLifecycleHandlers(t *testing.T) {
	...
	var called []string
	g := kplugin.NewGroup("test-plugin", 10)
	...
	g.OnSessionLifecycle(func(ctx context.Context, ev *session.LifecycleEvent) error {
		called = append(called, "session-lifecycle")
		return nil
	})
	g.OnToolLifecycle(func(ctx context.Context, ev *hooks.ToolEvent) error {
...
func TestPlugin_NilFieldsIgnored(t *testing.T) {
	...
	p := kplugin.BeforeLLMHook("partial", 0, func(ctx context.Context, ev *hooks.LLMEvent) error {
		return nil
	})
	installPlugin(reg, p)
...
func TestPlugin_OrderRespected(t *testing.T) {
	...
	var order []string
	installPlugin(reg, kplugin.BeforeLLMHook("late", 100, func(ctx context.Context, ev *hooks.LLMEvent) error {
		order = append(order, "late")
		return nil
	}))
	installPlugin(reg, kplugin.BeforeLLMHook("early", 1, func(ctx context.Context, ev *hooks.LLMEvent) error {
...
func TestPlugin_InterceptorWrapsHook(t *testing.T) {
	...
	g := kplugin.NewGroup("logger-like", 10)
	g.InterceptBeforeLLM(func(ctx context.Context, ev *hooks.LLMEvent, next func(context.Context) error) error {
...
func TestPlugin_RequiresName(t *testing.T) {
	p := kplugin.NewGroup("", 0)
	p.OnBeforeLLM(func(ctx context.Context, ev *hooks.LLMEvent) error { return nil })
	err := installPlugin(hooks.NewRegistry(), p)
	if err == nil {
...
func TestWithPlugin_InvalidPluginPanics(t *testing.T) {
	...
	New(WithPlugin(kplugin.NewGroup("invalid", 0)))
}

func TestWithPlugin_KernelOption(t *testing.T) {
	var called bool
	k := New(
		WithPlugin(kplugin.BeforeLLMHook("test", 0, func(ctx context.Context, ev *hooks.LLMEvent) error {
			called = true
			return nil
		})),
	)
...
func TestNewLLMAgent_InstallsPlugins(t *testing.T) {
	agent := NewLLMAgent(LLMAgentConfig{
		Name: "plugin-agent",
		Plugins: []Plugin{kplugin.BeforeLLMHook("before", 0, func(ctx context.Context, ev *hooks.LLMEvent) error {
