...
mossquant 是一个通用的 **有状态自主循环 Agent** 框架，通过可插拔的 Domain 适配器支持不同领域。
...
mossquant 的 Domain 模式适合所有 **持续运行、有状态、需要周期性感知和决策** 的 Agent 场景：
...
- 全局配置目录：`~/.mossquant`
- 全局配置文件：`~/.mossquant/config.yaml`

```yaml
# ~/.mossquant/config.yaml
provider: openai
model: gpt-4o
api_key: sk-...
