...
func (f *FailoverLLM) Complete(ctx context.Context, req port.CompletionRequest) (*port.CompletionResponse, error) {
	candidates, err := f.candidates(req.Config.Requirements)
	if err != nil {
		return nil, err
	}

	attempts := make([]port.LLMCallAttempt, 0, len(candidates))
	...
	for idx, candidate := range candidates {
		...
		for candidateRetry := 0; candidateRetry <= maxRetries; candidateRetry++ {
			...
			if err == nil {
				...
				attempts = append(attempts, port.LLMCallAttempt{
...
func (f *FailoverLLM) Stream(ctx context.Context, req port.CompletionRequest) (port.StreamIterator, error) {
	candidates, err := f.candidates(req.Config.Requirements)
	if err != nil {
		return nil, err
	}
...
func (f *FailoverLLM) candidates(req *port.TaskRequirement) ([]routedModel, error) {
...
func (f *FailoverLLM) handleBreakerOpen(model string, idx int, candidates []routedModel, attempts *[]port.LLMCallAttempt) (bool, error) {
...
type failoverStreamIterator struct {
	...
	attempts        []port.LLMCallAttempt
	currentIndex    int
	currentRetry    int
	currentIter     port.StreamIterator
	currentModel    string
	emitted         bool
...
func (it *failoverStreamIterator) Next() (port.StreamChunk, error) {
	for {
		...
		if it.emitted {
			...
			return port.StreamChunk{}, withMetadata(err, false, false, it.currentModel, it.Metadata().Attempts)
		}
		...
		if finalErr := it.handlePreEmissionError(err); finalErr != nil {
			return port.StreamChunk{}, finalErr
		}
	}
}
...
func (it *failoverStreamIterator) Metadata() port.LLMCallMetadata {
	meta := port.LLMCallMetadata{
		ActualModel: it.selectedModel,
		Attempts:    append([]port.LLMCallAttempt(nil), it.attempts...),
	}
	if strings.TrimSpace(meta.ActualModel) == "" {
		meta.ActualModel = it.currentModel
	}
	if provider, ok := it.currentIter.(port.MetadataStreamIterator); ok {
...
func (it *failoverStreamIterator) openCurrent() error {
	for {
		...
		sllm, ok := candidate.llm.(port.StreamingLLM)
		if !ok {
			fallbackErr := &port.LLMCallError{
				Err:          fmt.Errorf("model %q does not support streaming", candidate.profile.Name),
				Retryable:    false,
				FallbackSafe: true,
				Metadata:     port.LLMCallMetadata{ActualModel: candidate.profile.Name},
			}
...
func (it *failoverStreamIterator) recordSelectedAttempt() {
	...
	it.attempts = append(it.attempts, port.LLMCallAttempt{
		CandidateModel: it.currentModel,
		AttemptIndex:   it.currentIndex + 1,
		CandidateRetry: it.currentRetry,
		Outcome:        "selected",
...
func exhaustedFailoverError(err error, model string, attempts []port.LLMCallAttempt) error {
	return withMetadata(&failoverExhaustedError{
		cause:    err,
		attempts: append([]port.LLMCallAttempt(nil), attempts...),
	}, false, false, model, attempts)
}

func ensureResponseMetadata(resp *port.CompletionResponse, model string, attempts []port.LLMCallAttempt) *port.CompletionResponse {
	if resp == nil {
		return nil
	}
	base := port.LLMCallMetadata{
		ActualModel: model,
		Attempts:    append([]port.LLMCallAttempt(nil), attempts...),
	}
...
func mergeResponseMetadata(base, overlay port.LLMCallMetadata) port.LLMCallMetadata {
...
func withMetadata(err error, retryable, fallbackSafe bool, model string, attempts []port.LLMCallAttempt) error {
	if err == nil {
		return nil
	}
...
func (it *completionIterator) Next() (port.StreamChunk, error) {
	...
	if it.index < len(it.resp.ToolCalls) {
		...
		it.index++
		return port.StreamChunk{ToolCall: &call}, nil
	}
	if !it.sentDone {
		it.sentDone = true
...
func (it *completionIterator) Metadata() port.LLMCallMetadata { return it.metadata }
