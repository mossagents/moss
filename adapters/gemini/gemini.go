...
func (c *Client) effectiveModel(req port.CompletionRequest) string {
	if req.Config.Model != "" {
...
func (c *Client) buildConfig(req port.CompletionRequest, system *genai.Content) (*genai.GenerateContentConfig, error) {
...
func toGeminiTool(tools []port.ToolSpec) (*genai.Tool, error) {
...
func toGeminiContents(msgs []port.Message, defaultModel string) (*genai.Content, []*genai.Content, error) {
...
func collectToolCallNames(msgs []port.Message) map[string]string {
...
func toGeminiUserParts(parts []port.ContentPart, model string) ([]*genai.Part, error) {
	result := make([]*genai.Part, 0, len(parts))
	for _, part := range parts {
		switch part.Type {
		case port.ContentPartText:
			result = append(result, genai.NewPartFromText(part.Text))
		case port.ContentPartInputImage, port.ContentPartInputAudio, port.ContentPartInputVideo:
...
func toGeminiToolResultParts(results []port.ToolResult, nameByCallID map[string]string, model string) ([]*genai.Part, error) {
	...
	for _, tr := range results {
		...
		for _, cp := range tr.ContentParts {
			...
			case port.ContentPartOutputImage, port.ContentPartOutputAudio, port.ContentPartOutputVideo:
...
func toGeminiMediaPart(part port.ContentPart, role, model string) (*genai.Part, error) {
...
func toGeminiFunctionResponsePart(part port.ContentPart, model string) (*genai.FunctionResponsePart, error) {
	mimeType := strings.TrimSpace(part.MIMEType)
	if strings.TrimSpace(part.URL) != "" {
		if mimeType == "" {
			mimeType = inferMIMEFromURL(part.URL, part.Type)
...
func inferMIMEFromURL(url string, typ port.ContentPartType) string {
	...
	switch typ {
	case port.ContentPartInputImage, port.ContentPartOutputImage:
		return "image/jpeg"
	case port.ContentPartInputAudio, port.ContentPartOutputAudio:
		return "audio/wav"
	case port.ContentPartInputVideo, port.ContentPartOutputVideo:
		return "video/mp4"
	default:
...
func fromGeminiResponse(resp *genai.GenerateContentResponse) *port.CompletionResponse {
...
func fromGeminiCandidate(c *genai.Candidate) ([]port.ContentPart, []port.ToolCall) {
	...
	for i, p := range c.Content.Parts {
		...
		if strings.TrimSpace(p.Text) != "" {
			contentParts = append(contentParts, port.TextPart(p.Text))
		}
...
func inlinePartToContentPart(b *genai.Blob) port.ContentPart {
	mimeType := strings.TrimSpace(b.MIMEType)
	data := base64.StdEncoding.EncodeToString(b.Data)
	switch {
	case strings.HasPrefix(strings.ToLower(mimeType), "image/"):
		return port.MediaInlinePart(port.ContentPartOutputImage, mimeType, data, "")
	case strings.HasPrefix(strings.ToLower(mimeType), "audio/"):
		return port.MediaInlinePart(port.ContentPartOutputAudio, mimeType, data, "")
	case strings.HasPrefix(strings.ToLower(mimeType), "video/"):
		return port.MediaInlinePart(port.ContentPartOutputVideo, mimeType, data, "")
	default:
		return port.MediaInlinePart(port.ContentPartOutputImage, mimeType, data, "")
	}
}

func fromGeminiUsage(u *genai.GenerateContentResponseUsageMetadata) port.TokenUsage {
	if u == nil {
...
func (it *streamIterator) processResponse(resp *genai.GenerateContentResponse) {
	...
	for i, p := range c.Content.Parts {
		...
		if p.Text != "" {
			it.pending = append(it.pending, port.StreamChunk{Delta: p.Text})
		}
		if p.FunctionCall != nil {
			args, _ := json.Marshal(p.FunctionCall.Args)
			if len(args) == 0 {
