...
### 4.6 单一 canonical source + 明确合并键

同一条工具调用可能同时出现在多个事件流中：
...
- `ExecutionEvent` 是工具成败类信号的 canonical source；
- `ToolEvent` 只作为补充上下文来源；
- session 持久化事件只用于恢复，不作为新的 evidence source。
...
如果缺少这些键，则 candidate pipeline 不得把该事件计入 confidence，以避免同一工具结果被重复计数。
...
三层 scope 需要使用稳定身份键，而不是临时路径推断：

| Scope | Canonical Identity |
|---|---|
| `session` | `session_id` |
| `repo` | 规范化后的 workspace root + 可选 repo fingerprint |
| `user` | 当前应用数据目录下的用户级 identity |

规则：

- repo scope 不以当前 cwd 作为主键；
- workspace 路径变化时，应允许通过 repo fingerprint 迁移；
- user scope 不直接复用 repo / session 目录结构。
...
为了与当前 `ExtendedMemoryRecord` 兼容，本设计不新增第三套持久化状态机，而是复用现有：

- `Stage`: `manual` / `snapshot` / `consolidated` / `promoted`
- `Status`: `active` / `superseded` / `archived`

本文中的：

- `candidate` 表示逻辑阶段，不直接落为持久化 `Status`；
- `accepted` / `competing` 只作为 pipeline 内部判定；
- 真正写入 store 时，仅映射到现有 `Stage` / `Status` 组合。

但为了让 recall / 过期 / 冲突治理真正可执行，第一版必须把以下字段做成**可落库、可过滤、可索引的一等字段**，而不是仅靠自由格式 `Metadata`：
...
如果当前 `ExtendedMemoryRecord` / `ExtendedMemoryQuery` 无法直接表达这些字段，则本设计默认允许在 harness memory schema 中做增量扩展；
不接受只靠路径命名约定或 metadata 全表扫描来实现主召回链路。

同时，查询侧必须同步升级为 typed query contract。第一版至少需要支持以下 query fields：
...
- recall 主链路必须优先走 typed filters，而不是全文 query + 后过滤；
- promotion / dedupe 主链路也必须能够基于 `Scope + identity + Kind + Fingerprint` 做 typed 查询；
- TTL 必须在 query-time 生效，而不是召回后再人工剔除；
- `SortBy` 至少支持 `score`、`updated_at`、`last_used_at`；
- top-K 语义必须稳定，默认按 `SortBy` 降序后截断 `Limit`。
...
- `summary`
- `details`
- `stage`：snapshot / consolidated / promoted
- `status`：active / superseded / archived
- `confidence`
- `usage_count`
- `last_used_at`
...
recall 注入必须走 **turn-local `BeforeLLMHook`**，而不是只在 `NewSession` / `ComposeSystemPrompt` 时一次性拼入。

为避免实现再次退化为修改 `Session.Messages`，第一版必须先补齐一个明确的 request-local runtime contract。允许两种实现路径，二选一：

1. 扩展 `LLMEvent` 或等价 request object，使其显式携带本次 provider call 将发送的 request-local message buffer；
2. 在 prompt materialization 完成后、provider call 发出前，新增一个 pre-provider hook / interceptor，暴露 `[]model.Message` 的临时副本供 recall adapter 改写。

无论采用哪种路径，都必须满足：recall adapter 只能修改该 request-local message buffer，不能修改 `Session.Messages`，也不能修改任何会被 session persistence 保存的动态 prompt 载体。

要求：

- 每个 turn 都重新计算 recall；
- 注入内容只对当前 turn 生效；
- turn 结束后不得把 injected observations 持久化回 session 历史；
- 恢复旧 session 时，应重新 recall，而不是重放旧 recall 注入内容。

实现上必须满足一个更强的约束：

> recall 只能作用于**本次 LLM 请求的临时 prompt materialization 结果**，不能修改 `Session.Messages`，也不能写入任何会随 session 一起持久化的状态载体。

因此本设计不建议直接复用当前会改写 session 消息数组的 RAG 注入模式；  
应改为 request-scoped prompt assembly / cloned prompt view：

- 先复制并规范化本轮将发送给模型的 prompt messages；
- 在该临时副本上拼接 recall context；
- 把临时副本交给本次 provider call；
- 原始 `Session.Messages` 保持不变。

persistent session store 只保存：
...
不保存 turn-local recall 注入块。

如果 runtime 尚未提供上述 request-local contract，则本设计不得直接进入 recall 实现阶段。
...
### 10.5 replay / fork / restore 保护

以下事件默认不能再次生成新的 evidence：

- session restore
- checkpoint replay
- session fork 后对历史事件的重放

规则：

- 历史 materialized events 只能重建运行态，不能重新进入 candidate pipeline；
- 只有新的 root run / turn 中产生的实时事件，才允许沉淀新候选；
- replay / fork 若需写 memory，必须显式标记 `source_kind=replay` 并默认降权。
...
同时，接线形态应与当前 deep-agent preset / `harness.Feature` 边界一致：

- 记忆沉淀能力应作为可开关的 `harness.Feature` 或 preset pack 引入；
- 不应只落在某个产品入口私有 service 中；
- 必须支持 surface 级启停与回滚。
...
1. 注册 signal capture hooks；
2. 注册 candidate / promotion pipeline；
3. 注册 per-turn recall injection hook；
4. 暴露统一 kill switch 与调试开关。

默认应先在单一 surface 灰度启用，例如仅 `mosscode` TUI，再逐步扩展到 one-shot 与其他产品入口。
...
3. 恢复 / replay / fork 测试：
   - 恢复旧 session 不得重复摄取历史 evidence；
   - checkpoint replay 不得重复放大 usage_count / confidence；
   - fork 子线程后，父线程历史不得被当作新候选重新晋升。
...
6. 恢复、回放、fork 不会重复放大 historical evidence。
