...
- 恢复策略：
  ...
  - 连续损坏超过阈值（默认 100 条）：
    - `best-effort`：记录严重告警并继续恢复其余记录（默认）。
    - `strict`：中止恢复并上报错误。
...
- `CRON.json`：计划任务定义（可选）：
  - 缺失：按无定时任务处理（非错误）。
  - 格式错误：`strict` 模式启动失败；`best-effort` 模式跳过并告警。
- `skills/`：技能描述目录（可选，解析错误记录告警并跳过）。
- 启动时输出资产检查报告：`found/missing/invalid` 三类统计。

## 跨阶段依赖与契约冻结

- P1 依赖 P0 提供稳定的 `LaneQueue` 与 `DeliveryQueue` 基础接口。
- P2 依赖 P0 delivery 重试管线与 P1 routing/channel 元数据。
- 契约冻结点：
  - P0 完成后冻结 lane/delivery 接口主签名。
  - P1 完成后冻结 routing/channel 接口主签名。
  - P2 仅允许扩展配置项，不破坏已冻结签名。
