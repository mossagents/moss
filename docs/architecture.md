...
│   kernel\patterns (Agent 编排原语)                  │
│   appkit + harness\runtime                         │
...
| 层 | 主要职责 |
|---|---|
| `kernel\` | Agent 接口、`RunAgent`、Session、Event、Tool、Plugin、Model、UserIO、Workspace、Task、Observe、Checkpoint、Artifact |
...
| `harness\runtime\` | 默认能力装配与 execution/policy 基础设施：builtin tools、MCP、`SKILL.md`、subagent、context、memory、knowledge、scheduling |
| `harness\appkit\` | 面向应用的构建入口、扩展组合 API，以及 `BuildDeepAgent(...)` 产品预设路径 |
| `harness\extensions\` | 扩展能力命名空间：`skill\`、`mcp\`、`agent\`、`knowledge\`、`capability\`（五个子域）|
| `apps\` | 核心应用入口 |
| `examples\` | 参考实现与集成示例 |
...
- **Backend**：统一的部署后端抽象，围绕 `workspace.Workspace` 及其生命周期工作，并可选参与 factory / install / boot / shutdown。`LocalBackendFactory` + `LocalBackend` 是默认实现。
...
`apps\mosscode`、`examples\mossresearch`、`examples\mosswriter` 等产品面现在通过 `appkit.NewDeepAgentConfig(...)` 这一公开 preset contract 叠加差异化能力，而不是直接依赖内部 assembly 顺序。
...
当前扩展主要通过 Kernel 暴露的几个稳定注册面接入：

- `Stages()`：注册 boot / shutdown 生命周期阶段
- `PromptLayers()`：注入 system prompt layer
- `InstallPlugin()` / `hooks.Registry`：接入 LLM / tool / session lifecycle hook
- `Services()`：挂载受 owner package 管理的 substrate state

这比一个笼统的 “extension bridge” 更贴近当前实现，也更容易按职责拆分扩展边界。
