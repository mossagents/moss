...
| `harness\config\` | 配置、profile、模板上下文 |
...
| `harness\runtime\` | Layer 2 实现层：核心运行时装配；子包：`policy\`（工具策略编译与 Policy 接口）、`catalog\`（状态目录接口）、`memory\`（记忆存储实现）、`scheduling\`（调度实现）、`events\`（事件分发）、`state\`（StateCatalog 实现与适配器）、`profile\`（Profile 解析与 SessionPosture）、`probe\`（执行能力探测）、`knowledge\`（知识工具注册）、`thinking\`（思考工具实现）、`builtintools\`（内置工具处理器：exec、http、fs、ask_user）|
| `harness\internal\runtime\` | 内部实现细节（不可外部导入）：`assembly\`（装配链）、`capstate\`（能力状态绑定）、`runctx\`（执行上下文）、`execution\`（执行基础设施）、`planning\`（规划实现）|
