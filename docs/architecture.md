...
| 层 | 主要职责 |
|---|---|
| `kernel\` | Agent 接口、`RunAgent`、Session、Event、Tool、Plugin、Model、UserIO、Workspace/Executor、Task、Observe、Checkpoint |
...
`runtime\profile.go` 负责把 `profile + trust + approval` 解析为实际执行姿态。该文件已迁移至 `runtime\profile\`，`runtime\` 保留薄型 re-export 兼容层。
...
| `extensions\` | 扩展能力命名空间：`skill\`（`SKILL.md` 解析与发现）、`mcp\`（外部 MCP server 桥接）、`agent\`（委派代理注册与任务运行时协作）、`knowledge\`（知识库抽象）、`capability\`（扩展 Provider 注册与版本管理）|
| `runtime\` | Layer 2 实现层：核心运行时装配；子包：`policy\`（工具策略编译与 Policy 接口）、`catalog\`（状态目录接口）、`memory\`（记忆存储实现）、`scheduling\`（调度实现）、`events\`（事件分发）、`state\`（StateCatalog 实现与适配器）、`profile\`（Profile 解析与 SessionPosture）、`probe\`（执行能力探测）、`knowledge\`（知识工具注册）、`thinking\`（思考工具实现）、`builtintools\`（内置工具处理器：exec、http、fs、ask_user）|
| `internal\runtime\` | 内部实现细节（不可外部导入）：`assembly\`（装配链）、`capstate\`（能力状态绑定）、`runctx\`（执行上下文）、`execution\`（执行基础设施）、`planning\`（规划实现）|
| `scheduler\` | 调度器 |
