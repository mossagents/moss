...
| `harness\runtime\` | Layer 2 实现层；子包：`policy\`（工具策略编译）、`hooks\governance\`（policy/rbac/ratelimit/rag）、`hooks\declarative\`（YAML 驱动的声明式 hook）、`memory\`（记忆存储）、`scheduling\`（调度）、`events\`（事件分发）、`state\`、`profile\`、`builtintools\`|
