...
| `extensions\` | 扩展能力命名空间；当前包含 `skill\`、`mcp\`、`agent\`、`knowledge\` 四个子域 |
| `apps\` | 核心应用入口 |
| `examples\` | 参考实现与集成示例 |
...
| `extensions\` | 扩展能力命名空间：`skill\`（`SKILL.md` 解析与发现）、`mcp\`（外部 MCP server 桥接）、`agent\`（委派代理注册与任务运行时协作）、`knowledge\`（知识库抽象） |
| `scheduler\` | 调度器 |
