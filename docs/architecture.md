...
| `runtime\` | Layer 2 实现层：核心运行时装配；子包：`policy\`（工具策略编译与 Policy 接口）、`catalog\`（状态目录接口）、`memory\`（记忆存储实现）、`scheduling\`（调度实现）、`events\`（事件分发）、`state\`（StateCatalog 实现与适配器）、`profile\`（Profile 解析与 SessionPosture）、`builtintools\`（内置工具处理器：exec、http、fs、ask_user）|
